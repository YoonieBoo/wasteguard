  ...
  return (
    <main className="min-h-dvh w-full bg-[radial-gradient(circle_at_18%_10%,rgba(91,211,151,0.28),transparent_18rem),radial-gradient(circle_at_88%_20%,rgba(99,226,172,0.16),transparent_20rem),linear-gradient(180deg,#073f3f_0%,#0b322f_100%)] px-4 pb-28 pt-8 text-white sm:px-5 md:px-6 lg:px-8 lg:pb-8 lg:pt-7">
      <div className="mx-auto w-full max-w-[1040px]">
        ...
        <section className="rounded-[0.75rem] bg-white/[0.08] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.12)] backdrop-blur md:max-xl:mx-auto md:max-xl:w-full md:max-xl:max-w-[720px] md:max-xl:p-5 xl:p-6">
          ...
          <section className="border-t border-white/10 pt-5 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
            ...
            {role === 'owner' && (
              <button
                type="button"
                onClick={() => setShowGoalDetail((isOpen) => !isOpen)}
                className="mt-4 min-h-12 w-full rounded-[0.5rem] bg-white/[0.08] px-4 py-3 text-sm font-black text-emerald-100 transition hover:bg-white/[0.12]"
              >
            ...
            {role !== 'owner' && (
              <div className="mt-5">
                {isTodaySaved ? (
                  <p className="rounded-[0.5rem] bg-white/[0.08] px-4 py-3 text-center text-sm font-black text-emerald-200">
                    {t.resultSaved}
                  </p>
                ) : (
                  <button
                    onClick={onAddToday}
                    className="flex min-h-14 w-full items-center justify-center gap-2 rounded-[0.75rem] bg-emerald-400 px-4 py-3 text-center text-base font-black leading-tight text-emerald-950 shadow-[0_14px_30px_rgba(71,211,137,0.25)] transition hover:bg-emerald-300"
                  >
                    {t.addTodayResult}
